...
const frameworkData = [
  {
    ...
    imagePath: "/enterprice-deals/image-1.jpg", 
    icon: (
  ...
  },
  {
    ...
    imagePath: "/enterprice-deals/image-2.jpg", 
    icon: (
      <svg className="w-14 h-14 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
  ...
  },
  {
    ...
    imagePath: "/enterprice-deals/image-3.jpg", 
    icon: (
      <svg className="w-14 h-14 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
  ...
  },
  {
    ...
    imagePath: "/enterprice-deals/image-4.jpg", 
    icon: (
      <svg className="w-14 h-14 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
  ...
  return (
    <section className="w-full bg-[#F0EDE6] pb-24 px-6 flex flex-col items-center">
      ...
      <div className="w-full max-w-[1240px] grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch">
          ...
          {frameworkData.map((item, index) => {
            ...
            return (
              <button
                ...
                className={`flex-1 flex items-center gap-5 py-3 px-6 md:px-8 text-left transition-all duration-300 border-none outline-none min-h-[80px]
                  ${isActive 
                    ? "bg-[#161662] text-white shadow-xl" 
                    : "bg-[#E5E2DC] text-[#161662] hover:bg-[#DCD8D0]"
                  }`}
              >
                <div className={`${isActive ? "text-white" : "text-[#161662]"}`}>
                  {item.icon}
                </div>
        ...
        <div className="lg:col-span-7 bg-[#161662] w-full h-full p-6 md:p-10 flex flex-col items-start justify-start">
          <p className="text-white text-base md:text-lg font-medium text-left leading-relaxed mb-6 max-w-2xl flex-shrink-0">
            {frameworkData[activeIndex].description}
          </p>

          <div className="relative w-full flex-1 aspect-[16/10] bg-white shadow-2xl flex flex-col items-center justify-center overflow-hidden rounded-sm">
             {/* This is the placeholder for the Dashboard image as shown in screenshot */}
             <div className="text-center p-4 relative z-10">
               <p className="text-[#161662]/40 text-sm font-medium tracking-wide mb-2 uppercase">Image Placeholder</p>
               <p className="text-[#161662]/30 text-xs font-mono">{frameworkData[activeIndex].imagePath}</p>
             </div>
             
             {/* When user adds real images into public/enterprice-deals, they can add the Next.js Image component here:
                <Image src={frameworkData[activeIndex].imagePath} alt="Dashboard feature" fill className="object-cover" />
             */}
          </div>
        </div>
      </div>
