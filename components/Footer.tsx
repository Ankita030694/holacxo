      ...
      <div className="relative z-10 w-full max-w-7xl flex flex-col">
                ...
                <div className="flex flex-col gap-5">
                  {column.links.map((link, linkIdx) => (
                    <Link 
                      key={linkIdx} 
                      href={link.href} 
                      className="text-white/80 hover:text-white transition-colors text-[16px] w-fit active:text-[#3A6DFF] active:font-bold"
                    >
                      {link.name}
                    </Link>
                  ))}
        ...
        <div className="flex flex-col md:flex-row justify-end items-start md:items-end w-full mt-8 md:mt-12">
          ...
          <div className="flex gap-4">
            {/* LinkedIn icon */}
            <Link href="https://www.linkedin.com/company/holacxo" className="w-[50px] h-[50px] bg-[#F2F0EB] flex items-center justify-center hover:bg-white transition-colors active:bg-[#1A3FA8] active:scale-90">
